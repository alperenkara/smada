...
if __name__ == '__main__':
    ...
    n = 0
    try:
        fp = open("C:/Users/Joanna-T470/PycharmProjects/MatrixMultiplication/matrices-2.txt","r")
        line = fp.readline()
        dimensions = []
        rzad = []
        tablica = []
    ...
    #print("zakres", zakres)   #=6
    limity = list(range(0, numberOfMatrices, zakres))
    #print(limity, "długość", len(limity))   # [0, 6, 12, 18, 24, 30, 36, 42, 48] długość 9
    if limity[-1] < numberOfMatrices-1:
        limity.append(numberOfMatrices)
    #print("limity2", limity)                   # limity [0, 6, 12, 18, 24, 30, 36, 42, 48]
    ...
    for i in range(0,numberOfThreads):
        if i != numberOfThreads:
            #print("limity[i], limity[i+1]-1", i, i+1, limity[i], limity[i+1]-1)
            p = Process(target = multiplyArrays, args=(NpArrays, limity[i], limity[i+1]-1, output, i))
        else:
            # print("ostatnie limity[i], limity[i+1]", i, i+1, limity[i], limity[i+1])
            # print("LAST i: ---------------------->", i, i+1, limity[i], limity[i+1])
            # ostatnie limity[i], limity[i+1] 7 8 42 48
            # LAST i: ----------------------> 7 8 42 48
